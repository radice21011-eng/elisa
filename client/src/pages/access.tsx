...
export default function AccessPage() {
  ...
  const [error, setError] = useState(''); // Note: 'error' state variable is declared but not used in the original logic for displaying errors. It's kept for consistency with the original code.
  ...
  const handleAccess = async (e: React.FormEvent) => {
    ...
    // Add password validation if the email is the specific one
    if (email.toLowerCase() === 'ervin210@icloud.com' && !password) {
      toast({
        title: "Password Required",
        description: "Please enter the password for this authorized email address.",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="bg-black/40 border border-purple-500/30 rounded-2xl p-6 backdrop-blur-sm">
          <form onSubmit={handleAccess} className="space-y-6">
            ...
            <div>
              ...
              {email.toLowerCase() === 'ervin210@icloud.com' && (
                <div className="mt-4">
            ...
            <button
              type="submit"
              disabled={loading || !email || (email.toLowerCase() === 'ervin210@icloud.com' && !password)}
